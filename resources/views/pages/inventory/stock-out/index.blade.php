    ...
    <div class="mb-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <form method="GET" action="{{ route('stock-out.index') }}" class="flex flex-col gap-4 md:flex-row md:items-center">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('stock-out.index') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300">Refresh</a>
            </div>
        </form>
    </div>
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($trx->status === 'pending')
                                        <div class="flex justify-center gap-2">
                                            <form action="{{ route('stock-out.confirm', $trx) }}" method="POST" data-confirm-edit="Apakah Anda yakin ingin mengonfirmasi pengeluaran stok barang ini?" data-confirm-title="Konfirmasi Stok Keluar">
                                                @csrf
                                                <button type="submit" class="rounded-lg bg-green-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-green-700">
                                                    Konfirmasi
                                                </button>
                                            </form>
                                            <form action="{{ route('stock-out.reject', $trx) }}" method="POST" data-confirm-delete="Yakin tolak transaksi ini? Stok yang sudah dikurangi akan dikembalikan." data-confirm-title="Tolak Stok Keluar">
                                                @csrf
