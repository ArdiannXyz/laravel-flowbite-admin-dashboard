                                ...
                                <td class="px-6 py-4 text-center">
                                    @if($trx->status === 'pending')
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <button data-modal-target="reject-modal-in-{{ $trx->id }}" data-modal-toggle="reject-modal-in-{{ $trx->id }}" type="button" class="rounded-lg bg-red-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-red-700">
                                                Tolak
                                            </button>
                                        </div>

                                        <!-- Struktur Modal Flowbite (Barang Masuk) -->
                                        <div id="reject-modal-in-{{ $trx->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                                ...
                                                <div class="relative bg-white rounded-xl shadow dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-left">
                                                    <div class="flex items-center justify-between p-4 border-b rounded-t dark:border-gray-700">
                                                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                                            Alasan Penolakan Barang Masuk
                                                        </h3>
                                                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-700 dark:hover:text-white" data-modal-toggle="reject-modal-in-{{ $trx->id }}">
                                                    ...
                                                    <form action="{{ route('stock-in.reject', $trx) }}" method="POST" class="p-4 md:p-5">
                                                        @csrf
                                                        <div class="mb-4">
                                                            <label for="rejection_reason_in_{{ $trx->id }}" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                                                Masukkan Alasan Penolakan <span class="text-red-500">*</span>
                                                            </label>
                                                            <textarea name="rejection_reason" id="rejection_reason_in_{{ $trx->id }}" rows="3" class="w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-red-500 focus:ring-red-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white" placeholder="Contoh: Fisik barang rusak atau jumlah tidak sesuai..." required></textarea>
                                                        </div>
                                                        <div class="flex items-center justify-end gap-3">
                                                            <button type="button" data-modal-toggle="reject-modal-in-{{ $trx->id }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300">
                                                                Batal
                                                            </button>
